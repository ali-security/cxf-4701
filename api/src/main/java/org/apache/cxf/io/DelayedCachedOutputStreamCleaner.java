...
public final class DelayedCachedOutputStreamCleaner implements CachedOutputStreamCleaner, BusLifeCycleListener {
    ...
    private static class DelayedCleaner implements CachedOutputStreamCleaner, Closeable {
    ...
    private static final class DelayedCleanerImpl extends DelayedCleaner {
        ...
        @Override
        public void clean() {
            final Collection<DelayedCloseable> closeables = new ArrayList<DelayedCloseable>();
    ...
    private static final class DelayedCloseable implements Delayed {
        ...
        public int compareTo(Delayed o) {
            return Long.valueOf(expireAt).compareTo(Long.valueOf(((DelayedCloseable) o).expireAt));
        }
    ...
    public void setBus(Bus bus) {
        ...
        if (bus != null) {
            Object delayProp = bus.getProperty(CachedConstants.CLEANER_DELAY_BUS_PROP);
            if (delayProp instanceof Number) {
                delayValue = (Number) delayProp;
            } else if (delayProp instanceof String) {
                try {
                    delayValue = Long.valueOf((String) delayProp);
                } catch (NumberFormatException ex) {
                    // ignore, treat as not set
                }
            }

            Object shutdownProp = bus.getProperty(CachedConstants.CLEANER_CLEAN_ON_SHUTDOWN_BUS_PROP);
            if (shutdownProp instanceof Boolean) {
                cleanupOnShutdownValue = (Boolean) shutdownProp;
            } else if (shutdownProp instanceof String) {
                cleanupOnShutdownValue = Boolean.valueOf((String) shutdownProp);
            }

            busLifeCycleManager = bus.getExtension(BusLifeCycleManager.class);
            bus.setExtension(this, CachedOutputStreamCleaner.class);
        }
        ...
        if (cleanupOnShutdownValue != null) {
            cleanupOnShutdown = cleanupOnShutdownValue.booleanValue();
        } else {
